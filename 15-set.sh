#!/bin/bash
#sudo id -u = 0
set -e  #this is a signal to indicate error.
...
Y="\e[32m"
N="\e[0m" 
trap 'echo "error at $LINENO, command: $BASH_COMMAND"' ERR #trap is a command, when the error spots ERR will get the signal & trap will execute.
#trap is unexpected error
if [ $USERID -ne 0 ]; then
